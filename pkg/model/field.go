...
// HasCustomSync returns true if the field is configured with `custom_sync`,
// meaning it is reconciled by a hand-written sync function rather than by the
// resource's Update operation.
func (f *Field) HasCustomSync() bool {
...
// CustomSyncAppliedOnCreate returns true if the field is configured with
// `custom_sync.applied_on_create`, meaning the resource's Create operation
// already applies it and no post-create sync is pending for it.
//
// This governs the post-create marker only. The field is still synced on update.
func (f *Field) CustomSyncAppliedOnCreate() bool {
	return f.HasCustomSync() && f.FieldConfig.CustomSync.AppliedOnCreate
}
