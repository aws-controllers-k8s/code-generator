...
func TestWAFv2_RenamesAndReferenceFields(t *testing.T) {
	assert := assert.New(t)
	require := require.New(t)
	...
	// TypeDef-level assertion for rename propagation
	typeDef := testutil.GetTypeDefByName(t, g, "IPSetReferenceStatement")
	require.NotNil(typeDef)
	assert.NotNil(typeDef.GetAttribute("IPSetARN"))
	assert.Nil(typeDef.GetAttribute("ARN"))

	typeDef = testutil.GetTypeDefByName(t, g, "RuleGroupReferenceStatement")
	require.NotNil(typeDef)
	assert.NotNil(typeDef.GetAttribute("RuleGroupARN"))
	assert.NotNil(typeDef.GetAttribute("RuleGroupRef"))
	assert.Nil(typeDef.GetAttribute("ARN"))

	// Ensure both resources include the shared IPSetReferenceStatement path.
	// (The nested members are validated via the shared TypeDef above.)
	for _, crdName := range []string{"RuleGroup", "WebACL"} {
		crd := getCRDByName(crdName, crds)
		require.NotNil(crd, "CRD %s not found", crdName)

		rulesField := crd.Fields["Rules"]
		require.NotNil(rulesField, "%s.Rules field not found", crdName)

		stmtField := rulesField.MemberFields["Statement"]
		require.NotNil(stmtField, "%s.Rules.Statement field not found", crdName)

		ipsrsField := stmtField.MemberFields["IPSetReferenceStatement"]
		require.NotNil(ipsrsField, "%s.Rules.Statement.IPSetReferenceStatement field not found", crdName)
	}
	...
	rgrsField := stmtField.MemberFields["RuleGroupReferenceStatement"]
	require.NotNil(rgrsField, "%s.Rules.Statement.RuleGroupReferenceStatement field not found", crdName)
}
